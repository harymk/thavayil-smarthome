...
app.post('/oauth/authorize', async (req,res)=>{
  try{
    ...
    let finalUrl; if(redirect_uri.includes('?')) finalUrl = `${redirect_uri}&code=${code}&state=${state}`; else finalUrl = `${redirect_uri}?code=${code}&state=${state}`;
...
app.post('/oauth/token', async (req,res)=>{
  try{
    console.log('OAUTH TOKEN REQ:', {grant_type:req.body.grant_type, client_id:req.body.client_id, code:!!req.body.code, refresh:!!req.body.refresh_token});
...
app.post('/alexa/smarthome', async (req,res)=>{
  try{
    ...
    if(ns==='Alexa.Discovery' && name==='Discover'){
      try{ await OfflineState.deleteMany({}); global.offlineDevices.clear(); await Device.updateMany({userId}, {offline:false}); }catch(e){}
...
// V55: Alias for Alexa/Google - both URLs work
app.post('/smarthome', async (req,res)=>{
  console.log('ALIAS /smarthome called -> forwarding to /google/smarthome logic');
  req.url='/google/smarthome';
  // Re-use google handler by calling next - simplest: duplicate call
  // For single file, just call same function
  try{
    const auth=req.headers.authorization; if(!auth) return res.status(401).json({error:'no auth'});
    const token=auth.replace('Bearer ',''); let decoded; try{ decoded=require('jsonwebtoken').verify(token, 'thavayil-smarthome-secret-2024-fixed'); }catch(e){ try{ decoded=require('jsonwebtoken').verify(token, 'my-super-secret-123-change-this'); }catch(e2){ return res.status(401).json({error:'invalid token'}); } }
    const userId=decoded.userId;
    ...
    if(intent==='action.devices.SYNC'){
      // V57: Clear ALL offline marks - both collections and Device field
      try{ await OfflineState.deleteMany({}); global.offlineDevices.clear(); await Device.updateMany({userId}, {offline:false}); console.log('V57 cleared offline flags in DB'); }catch(e){ console.log('clear offline error', e.message); }
    ...
    // For QUERY/EXECUTE, forward to main handler logic - simplified return empty to avoid double
    return res.status(404).json({error:'use /google/smarthome'});
  }catch(e){ console.log('ALIAS ERROR', e.message); return res.status(500).json({error:e.message}); }
});
  ...
  try{
    ...
    if(intent==='action.devices.QUERY'){
      ...
      for(const q of payloadDevices){
        ...
        let online = true; // V57 FORCE ONLINE ALWAYS
